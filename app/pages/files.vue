...
const viewFile = (fileName) => window.open(`/uploads/${fileName}`, '_blank')
...
const deleteFile = async (fileId, fileName) => {
    ...
    try {
        const response = await $fetch('/api/files/delete', { 
            method: 'POST', 
            body: { fileId, fileName, username: username.value } 
        })
              ...
              <tr v-for="file in files" :key="file.id">
                ...
                <td>
                  <div class="action-btns">
                    <button class="btn-i" @click="viewFile(file.name)">👁️</button>
                    <button class="btn-i" title="Kopyala" @click="copyFileToClipboard(file.name)">📋</button>
                    <button class="btn-i" @click="downloadFile(file.name)">📥</button>
                    <button class="btn-i btn-red" @click="deleteFile(file.id, file.name)  ">🗑️</button>
                  </div>
                </td>
              </tr>
            </tbody>
...
<style scoped>
/* TÜM EKRAN VE SIDEBAR SABİTLEME */
.dashboard-wrapper {
  display: flex;
  width: 100%;
  min-width: 100%;
...
.nav-link {
  ...
  color: #a1a1aa; text-decoration: none !important; border-radius: 12px; margin-bottom: 8px;
}
...
/* TABLO KARTI */
.content-card {
  background: #16161a; border: 1px solid #232329; border-radius: 24px; overflow: hidden;
}

.file-table { width: 100%; border-collapse: collapse; text-align: left; }
.file-table th { padding: 20px 24px; font-size: 13px; color: #63636e; border-bottom: 1px solid #232329; }
...
.btn-i {
  background: #1f1f24; border: 1px solid #2d2d35; color: white;
...
/* Tablo ve diğer stiller (Öncekiyle aynı) */
.content-card { background: #16161a; border: 1px solid #232329; border-radius: 24px; overflow: hidden; }
.file-table { width: 100%; border-collapse: collapse; }
.file-table th { padding: 20px 24px; color: #63636e; font-size: 13px; text-align: left; }
.file-table td { padding: 18px 24px; border-bottom: 1px solid #1f1f24; color: #a1a1aa; }
.avatar-box { width: 45px; height: 45px; background: #6366f1; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-weight: bold; }
</style>